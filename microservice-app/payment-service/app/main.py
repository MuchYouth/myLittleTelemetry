...
# --- 서비스 이름 설정 ---
service_name = "payment-service"  # 서비스마다 이름 변경해야 함
...
async def pay(request: Request):
    span = get_current_span()
    trace_id = format(span.get_span_context().trace_id, '032x') if span else "unknown"
    if random.random() < 0.15:  # 15% 확률로 결제 실패
        logger.error("Payment failed due to insufficient funds", extra={"trace_id": trace_id, "user_id": "user-123"})
        raise HTTPException(status_code=402, detail="Payment Required")
    log_request("/pay", request)
...
async def refund(request: Request):
    span = get_current_span()
    trace_id = format(span.get_span_context().trace_id, '032x') if span else "unknown"
    if random.random() < 0.3:  # 30% 확률로 환불 실패
        logger.error("Refund request failed due to system error", extra={"trace_id": trace_id, "user_id": "user-123"})
        raise HTTPException(status_code=500, detail="Refund failed")
    log_request("/refund", request)
    return {"message": "Refund successful"}

@app.get("/status")
async def payment_status(request: Request):
    log_request("/status", request)
    return {"payment_id": "pay-789", "status": "completed"}
